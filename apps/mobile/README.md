...
- 全屏阅读器：Markdown 渲染、字号调节、目录、AI 导读卡、图片全屏预览、阅读位置记忆、划词批注（高亮/笔记/编辑/删除）、即时 AI 问答（底部弹层）
- 知识库问答（第三 tab）：会话列表（新建/删除/切换）、流式回答、引用来源跳原文
- 分享接收：系统分享 text/plain 进 App，弹确认框导入（链接走 URL 导入、纯文本走摘录导入）
- 外观：浅色 / 深色 / 跟随系统三档（设置页切换），代码高亮主题联动
- 设置：服务器地址、账号退出
...
选区检测用 `selectionchange` + 防抖（Web 端绑定的 `@mouseup` 在触屏上不触发）。
以下场景只能在真机/模拟器上验证（桌面浏览器没有长按选择与选区句柄）：

1. 长按选中正文文字 → 浮条出现在选区上方（顶部空间不足翻到下方），与系统「复制/分享」菜单不互相遮挡
2. 拖动选区句柄扩大/缩小选区 → 松手后浮条位置正确，点「高亮」直接创建、点「写笔记」弹层保存
3. 在已有高亮附近划词（重叠区间）→ toast「该区域已有高亮」
4. 批注列表条目左滑 → 「笔记」编辑保存、「删除」带确认弹窗；点条目本体滚动定位并闪烁高亮
5. 深色模式下高亮标记与浮条对比度正常
6. 滚动页面时浮条自动收起，不会残留在错误位置
